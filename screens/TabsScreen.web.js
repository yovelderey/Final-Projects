...
import { View, Text, FlatList,ScrollView, StyleSheet, TextInput, TouchableOpacity } from 'react-native';
  ...
  if (user) {
    ...
    onValue(whatsappRef, (snapshot) => {
      ...
      Object.values(messages).forEach((msg) => {
        ...
        if (msg.status === "sent") {
          updatedCategories["נשלח"].push(contactDisplay);
        } else if (msg.status && msg.status.startsWith("error")) { // ✅ תנאי מתוקן
          updatedCategories["לא נשלח"].push(contactDisplay);
        }
      });
...
useEffect(() => {
 ...
 useEffect(() => {
    ...
    if (snapshot.exists()) {
      ...
      Object.entries(messages).forEach(([msgKey, msg]) => {
        if (msg.status && msg.status.startsWith('error')) {
          updates[msgKey + '/status'] = 'pending';
        }
      });

      if (Object.keys(updates).length > 0) {
        firebase.database().ref(`whatsapp/${user.uid}/${id}`).update(updates);
      }

      setResendTrigger(false); // מחזירים את הטריגר למצב false אחרי הסיום
    }
  }, { onlyOnce: true });

}, [resendTrigger, user, id]);
  ...
  /** 🟢 הצגת נתונים */
  const handleResendSingle = (phone) => {
  const whatsappRef = ref(database, `whatsapp/${user.uid}/${id}`);
  onValue(whatsappRef, (snapshot) => {
    if (snapshot.exists()) {
      const messages = snapshot.val();
      Object.entries(messages).forEach(([msgKey, msg]) => {
        if (normalizePhoneNumber(msg.formattedContacts) === phone && msg.status.startsWith('error')) {
          firebase.database().ref(`whatsapp/${user.uid}/${id}/${msgKey}`).update({ status: 'pending' });
        }
      });
    }
  }, { onlyOnce: true });
    ...
    <View style={[
      ...
      {activeCategory === "לא נשלח" && (
        <TouchableOpacity
          ...
          onPress={() => handleResendSingle(phone)}
        >
          <Text style={{ color: "#FFFFFF", fontSize: 12 }}>שלח שוב</Text>
        </TouchableOpacity>
      )}
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ padding: 10, paddingBottom: 40 }}>
        ...
        {activeCategory === 'לא נשלח' && (
          <TouchableOpacity
            style={styles.resendAllBtn}
            onPress={() => setResendTrigger(true)}
          >
            <Text style={styles.resendAllBtnText}>שלח שוב את כל ההודעות שנכשלו</Text>
          </TouchableOpacity>
        )}

        {/* סה״כ מגיעים */}
        {activeCategory === 'מגיעים' && (
          <Text style={styles.totalGuestsText}>סך הכל מגיעים: {getTotalGuests()}</Text>
        )}
...
totalGuestsText: {
  ...
  resendAllBtn: {
    ...
    paddingHorizontal: 15,
    borderRadius: 8,
    margin: 10,                // ⬅️ חזרה ל-margin מלא (לא רק vertical)
    alignItems: 'center',
