...
const SplashScreen = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (isConnected) {
        navigation.replace('Main'); // 'Main' הוא שם המסך הראשי באפליקציה שלך
      }
    }, 4500); // זמן בסרטונים באלפיות שנייה (4.5 שניות)
  ...
  return (
    <View style={styles.container}>
      <Image
        source={require('../assets/imgload.png')} // נתיב נכון לתמונה המקומית
        style={styles.backgroundImage}
      />
      <View style={styles.loadingBarContainer}>
        <Animated.View
          style={[styles.loadingBar, { width: loadingBarWidth.interpolate({
            inputRange: [0, 100],
            outputRange: ['0%', '100%'],
          }) }]}
        />
