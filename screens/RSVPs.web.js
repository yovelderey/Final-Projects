...
const waitingList = contacts.filter(c => {
  const res = responses[c.recordID || c.id]?.response;
  return !res || res === 'טרם השיבו';
});
...
const getRecipients = () => {
  switch (targetGroup) {
    ...
    case 'waiting':
      return contacts.filter(c => {const res = responses[c.recordID || c.id]?.response; return !res || res === 'טרם השיבו';});
...
<View style={styles.choiceRow}>
  {[
    ...
    { key: 'waiting',   label: `בהמתנה (${waitingList.length})` }, 
